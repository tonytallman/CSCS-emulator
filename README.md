...
Bike Sensor Emulator simulates a Bluetooth Low Energy cycling speed and cadence sensor, allowing developers, testers, and users to test CSCS-compatible apps without dedicated hardware. The app advertises itself as a standard BLE peripheral (localized name; always includes **CSCS**) and publishes simulated measurement data according to the Bluetooth SIG CSCS specification.

## Supported Languages

The app UI is localized for English, Spanish, Simplified Chinese, Traditional Chinese, Japanese, German, French, Portuguese (Brazil), Korean, and Italian. The Bluetooth SIG acronym **CSCS** is never translated.
...
- **Advertised name:** Localized; always includes `CSCS` (English: `CSCS Emulator`)
...
To prepare a new build, use the **`prepare-app-store-build`** Cursor skill (`.cursor/skills/prepare-app-store-build/`) or run the scripts under `scripts/` — bump build number, run unit tests, archive into Xcode Organizer (Validate/Distribute from Organizer; no repo archive or `.ipa` export), capture screenshots (named `Screenshot iPhone` / `Screenshot iPad` Simulators), upload localized App Store listing copy via App Store Connect API (`scripts/upload-app-store-localizations.py`), check [tallmansoftware.com](https://www.tallmansoftware.com/) for website copy drift (writes `documentation/WEBSITE_UPDATES.md` when updates are needed), then commit and tag locally (does not push). Archive upload and submit-for-review remain manual; App Review recording is manual.
